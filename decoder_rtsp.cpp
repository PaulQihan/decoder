...
double synchronize_video(VideoState* is, AVFrame* src_frame, double pts) {
    ...
    //得到video_clock,实际上也是预测的下一帧视频的时间
...
int demux_thread(void* arg) {
    ...
    av_dict_set(&optionsDict, "buffer_size", "1024000000", 0); //设置缓存大小，1080p可将值调大
    av_dict_set(&optionsDict, "rtsp_transport", "tcp", 0);                //采用tcp传输	,,如果不设置这个有些rtsp流就会卡着,防止丢包
